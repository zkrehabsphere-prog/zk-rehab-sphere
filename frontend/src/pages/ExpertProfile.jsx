...
import { 
  ...
  MapPin, Globe, Linkedin, Twitter, Facebook, User 
} from 'lucide-react';
...
const ExpertProfile = () => {
  ...
  if (error || !expert) return <div className="min-h-screen flex items-center justify-center p-4"><div className="text-center"><h2 className="text-xl font-bold text-slate-800 mb-2">{error}</h2><button onClick={() => navigate('/experts')} className="text-primary font-bold">Back to Experts</button></div></div>;

  return (
    <div className="min-h-screen bg-white pt-24 pb-12">
      <SEO title={`${expert.name} — Pediatric Expert`} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-32 h-fit">
            <div className="rounded-[2.5rem] overflow-hidden shadow-2xl shadow-slate-200/50 border-4 border-white mb-6 aspect-[4/5]">
              <img 
                src={expert.image || '/placeholder.jpg'} 
                alt={expert.name} 
            ...
            <div className="space-y-4">
              ...
              {expert.socialLinks && Object.values(expert.socialLinks).some(v => v) && (
                <div className="flex justify-center gap-4 py-2">
                  {expert.socialLinks.linkedin && <a href={expert.socialLinks.linkedin} className="p-3 bg-slate-50 text-slate-400 hover:text-blue-600 rounded-full transition-colors"><Linkedin size={18} /></a>}
                  {expert.socialLinks.twitter && <a href={expert.socialLinks.twitter} className="p-3 bg-slate-50 text-slate-400 hover:text-blue-400 rounded-full transition-colors"><Twitter size={18} /></a>}
                  {expert.socialLinks.facebook && <a href={expert.socialLinks.facebook} className="p-3 bg-slate-50 text-slate-400 hover:text-blue-700 rounded-full transition-colors"><Facebook size={18} /></a>}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-8 space-y-10">
            <div>
              ...
              <h1 className="text-5xl font-black text-slate-900 mb-2">{expert.name}</h1>
              <p className="text-lg text-slate-600 font-medium">{expert.role} — Expert in Child Rehabilitation</p>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 group hover:border-primary/30 transition-colors">
                ...
                <p className="text-slate-800 font-black">{expert.degree}</p>
              </div>
              <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 group hover:border-primary/30 transition-colors">
                ...
                <p className="text-slate-800 font-black">{expert.experience}</p>
              </div>
            ...
            <div>
              <h3 className="text-2xl font-black text-slate-800 mb-4 flex items-center gap-2">
                <User size={24} className="text-primary" /> About {expert.name}
              </h3>
      ...
      <BookingModal isOpen={isBookingOpen} onClose={() => setIsBookingOpen(false)} selectedExpertId={expert.linkedUserId?._id || expert.linkedUserId || expert._id} />
    </div>
  );
};
